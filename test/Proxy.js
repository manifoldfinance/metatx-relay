...
contract('Proxy', (accounts) => {
  ...
  before(async () => {
    ...
    smartWalletContract = await SmartWallet.new(ZERO_ADDRESS, ZERO_ADDRESS, {
      from: RELAYER,
    });
    forwarderContract = await Forwarder.new(ZERO_ADDRESS, [ZERO_ADDRESS], {
      from: RELAYER,
    });
    proxyFactoryContract = await ProxyFactory.new({ from: RELAYER });
    dummyForwarderContract = await DummyForwarder.new(
      ZERO_ADDRESS,
      [ZERO_ADDRESS],
      { from: RELAYER },
    );
  ...
  describe('Forwarder', async () => {
    ...
    beforeEach(async () => {
      proxyForwarder = await deployProxy(
        forwarderContract.address,
        forwarderContract.contract.methods
          .initialize(EOAs[1].address, authorizedRelayersContract.address, [])
          .encodeABI(),
        { from: RELAYER },
      );
      forwarder = await Forwarder.at(proxyForwarder.address);
      proxySmartWallet = await deployProxy(
        smartWalletContract.address,
        smartWalletContract.contract.methods
          .initialize(EOAs[1].address, forwarder.address)
          .encodeABI(),
        { from: RELAYER },
      );
    ...
    it('should have an implementation', async () => {
      assert.equal(
        await proxyForwarder.implementation(),
        forwarderContract.address,
      );
    });
    ...
    it('should change implementation', async () => {
      ...
      let tx = await signer.signTransaction({
        ...
        gas: 300000,
        gasPrice: 0,
        data: await forwarder.contract.methods
          .upgradeTo(dummyForwarderContract.address)
          .encodeABI(),
      });
      ...
      assert.equal(
        await forwarder.implementation(),
        dummyForwarderContract.address,
      );
