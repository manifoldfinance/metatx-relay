...
contract SmartWallet is Implementation, OwnersMap, ISmartWallet, ERC165 {
    ...
    function upgradeTo(address newImplementation) public onlyOwners {
    ...
    function execute(
        ...
        require(executeCall(gasleft(), to, value, data), "call failed");
    ...
    ) external override onlyOwners returns (address) {
    ...
    function batch(Call[] memory calls) public onlyOwners {
        for (uint256 i = 0; i < calls.length; i++) {
            execute(calls[i].to, calls[i].value, calls[i].data);
        }
    }
