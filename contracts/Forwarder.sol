...
contract Forwarder is Implementation, OwnersMap {
    ...
    constructor(address relayersAddress, address[] memory _trustedContracts)
        public
    {
    ...
    function getNonce(address signer, uint128 channel)
        external
        view
        returns (uint128)
    {
    ...
    function forward(
        ...
        bytes32 _hash = hashTxMessage(signer, to, data, nonce);
        ...
        uint256 forwardGasPrice = tx.gasprice < gasPriceLimit
            ? tx.gasprice
            : gasPriceLimit;
    ...
    function signerIsValid(
        ...
        if (v != 27 && v != 28) {
            ...
            bytes32 digest = keccak256(
                abi.encodePacked("\x19\x01", domainSeparator, message)
            );
        ...
        returns (bytes32)
    {
        return
            keccak256(
                abi.encode(EIP712DOMAIN_TYPEHASH, _verifyingContract, _chainId)
            );
    }
}
